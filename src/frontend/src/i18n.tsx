...
const catalogs = {
  "zh-CN": {
    ...
    agent: {
      ...
      sessionSettingsDescription: "模型和推理强度按会话保存，只影响当前会话后续发送的消息。",
      ...
      sessionSettingsHint: "这里展示的是会话级设置，不会再覆盖其他会话。",
      sessionPanelFields: {
        status: "状态",
        activeProject: "当前工程",
        container: "容器",
    ...
    admin: {
      ...
      configTitle: "运行配置",
      configDescription: "这里仅管理 Copilot SDK 的运行环境，比如工作目录、CLI 路径和代理。模型与推理强度现在在具体会话里单独保存。",
  ...
  "en-US": {
    ...
    agent: {
      ...
      sessionSettingsDescription: "Model and reasoning effort are stored per session and only affect future messages in this chat.",
      sessionSettingsFields: {
        model: "Model",
      ...
      sessionSettingsHint: "These settings belong to the current session only and do not affect other chats.",
      sessionPanelFields: {
        status: "Status",
    ...
    admin: {
      ...
      configDescription: "Manage only the Copilot SDK runtime environment here, such as the workspace root, CLI path, and proxy settings. Model and reasoning effort now live on each individual session.",
