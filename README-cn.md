...
| 维度 | 初步设想 |
| --- | --- |
| **任务集** | 约 500 题，分 5 类：*keyframe · mask · expression · comp-build · render-export*。每题 = 自然语言 prompt + 参考视频 + 一份 ground-truth `.aep`。 |
| **评分** | 视觉相似度（LPIPS / DreamSim 对照参考渲染）+ 结构匹配（comp / layer / keyframe 数量）+ wall-time 与 token cost + 完成率，合并成一个 0–100 的分数。 |
| **Ground truth** | 由几位资深 motion designer 各做一遍；保留多份解，不强行选出唯一"正确答案"，避免假装创意工作存在唯一解。 |
| **分发** | GitHub Pages leaderboard。每次提交带分数、`.aep`、渲染 mp4 和复现命令 —— 所有产物公开可查。 |
